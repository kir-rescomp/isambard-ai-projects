...
class WeightedEnsemble:
    ...
    def __init__(
        ...
        self.generator.manual_seed(config.seed + 7919 * self.comm.rank)

        self.basin_a = basin_a_states.to(self.device)
        n_local = max(1, config.initial_walkers // self.comm.world_size)
        idx = torch.randint(
            0, self.basin_a.shape[0], (n_local,), device=self.device, generator=self.generator
        )
        self.x = self.basin_a.index_select(0, idx).clone()
        self.w = torch.full(
            (n_local,), 1.0 / (n_local * self.comm.world_size), dtype=torch.float64, device=self.device
        )
