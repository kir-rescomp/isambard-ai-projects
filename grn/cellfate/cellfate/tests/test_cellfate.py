...
def test_birth_death_stationary_moments_match_poisson():
    net = build("birth_death", k=20.0, d=1.0)
    eng = TorchEngine(net, device="cpu")
    x = torch.zeros((20000, 1), dtype=torch.int64)
    x, diag = eng.advance(x, 0.02, 1500)
    xs = x[:, 0].double()
    assert abs(float(xs.mean()) - 20.0) < 0.4
    assert abs(float(xs.var()) - 20.0) < 1.2
