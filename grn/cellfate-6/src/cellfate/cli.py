...
from .dist.slurm import bootstrap_from_slurm, nccl_library_in_use
...
def pick_device(arg: str) -> torch.device:
    ...
    if torch.cuda.is_available():
        ...
        return torch.device(f"cuda:{int(local) % torch.cuda.device_count()}")
...
def cmd_selftest(args) -> int:
    ...
    # 2. birth-death stationary moments.
    #
    # The exact process is Poisson(k/d), but tau-leaping does not reproduce it
    # exactly and the test must target the scheme's own stationary moments, not
    # the exact ones.  For x' = x + Pois(k.tau) - Pois(d.x.tau),
    #
    #     E[x]   = k/d                              (exact in the mean)
    #     Var[x] = 2k.tau + (1 - d.tau)^2 Var[x]
    #            = (k/d) / (1 - d.tau/2)
    #
    # so the variance is inflated by a factor 1/(1 - d.tau/2), which is 1.01 at
    # tau = 0.02.  Verified numerically for tau from 0.005 to 0.1.  Tolerances
    # are 5 sampling standard errors, using the Poisson fourth moment
    # Var(s^2) = (lambda + 2.lambda^2)/n, so the test tightens automatically as
    # --walkers rises rather than staying loose enough to hide real bias.
    k_bd, d_bd, tau_bd = 20.0, 1.0, 0.02
    net = models.birth_death(k=k_bd, d=d_bd).compile()
    eng = TorchEngine(net, device=device)
    g = torch.Generator(device=device)
    g.manual_seed(20260902)  # deterministic: a moment check must not be a coin flip
    x = torch.zeros((args.walkers, 1), dtype=torch.int64, device=device)
    x, diag = eng.advance(x, tau_bd, 1500, generator=g)
    m, v = float(x.double().mean()), float(x.double().var())

    lam = k_bd / d_bd
    v_pred = lam / (1.0 - d_bd * tau_bd / 2.0)
    se_m = 5.0 * np.sqrt(v_pred / args.walkers)
    se_v = 5.0 * np.sqrt((lam + 2.0 * lam**2) / args.walkers)
    check("birth_death/mean", abs(m - lam) < se_m, f"{m:.3f} vs {lam:.3f} +/- {se_m:.3f}")
    check("birth_death/variance", abs(v - v_pred) < se_v,
          f"{v:.3f} vs tau-leap prediction {v_pred:.3f} +/- {se_v:.3f} "
          f"(exact-process value {lam:.3f})")
...
def run_one(conf: dict, device, comm, out_path: Path | None, verbose: bool = True):
    ...
    if comm.rank == 0 and out_path is not None:
        p = out_path
        if comm.n_groups > 1:
            p = out_path.with_name(f"{out_path.stem}_replica{comm.group_id:03d}{out_path.suffix}")
        write_run(p, summary, we.logs, conf.get("we", {}), mparams, acc.transitions)
    return summary
...
def cmd_run(args) -> int:
    ...
    replicas = int(conf.get("we", {}).get("replicas", 1))
    if args.replicas:
        replicas = args.replicas
    comm = make_communicator(device=device, replicas=replicas)
    if args.override:
    ...
    if comm.n_groups > 1:
        # Replicas are independent, so their spread is the honest error bar.
        results = comm.gather_group_results({"mfpt": summary["mfpt"], "flux": summary["flux"]})
        fluxes = np.array([r["flux"] for r in results], dtype=float)
        good = fluxes[np.isfinite(fluxes) & (fluxes > 0)]
        if getattr(comm, "global_rank", comm.rank) == 0 and good.size:
            mean_flux = good.mean()
            se = good.std(ddof=1) / np.sqrt(good.size) if good.size > 1 else float("nan")
            print(
                f"\ncombined over {good.size} replicas: "
                f"MFPT {1/mean_flux:.4e} +/- {se/mean_flux**2:.2e}   flux {mean_flux:.4e}"
            )
            for r in results:
                print(f"  replica {r['group']:3d}: MFPT {r['mfpt']:.4e}")
...
def main(argv=None) -> int:
    ...
    p.add_argument("--replicas", type=int, default=0,
                   help="independent ensembles to run concurrently; overrides we.replicas")
    ...
    # Accept either launcher: torchrun sets RANK/WORLD_SIZE itself, srun does
    # not, so derive them from the Slurm environment when they are absent.
    bootstrap_from_slurm()
